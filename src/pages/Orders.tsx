...
import { Search, Package, Clock, CheckCircle2, ChevronRight, Calendar, Phone, House, RotateCcw } from 'lucide-react';
...
export default function Orders() {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // If a specific order number is provided, search for it specifically (allows guests)
      if (orderQuery && orderQuery.length >= 6) {
        const q = query(
          collection(db, 'orders'),
          where('orderNumber', '==', orderQuery.trim()),
          limit(1)
        );
        const snapshot = await getDocs(q);
        ordersList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any));
      } 
      // If logged in and no query, show user's recent orders
  ...
  // Auto-load orders on mount if logged in
  React.useEffect(() => {
        ...
        <div className="bg-slate-900 rounded-[50px] p-12 text-white relative overflow-hidden shadow-2xl">
           <div className="relative z-10">
              ...
              <p className="text-slate-400 text-sm max-w-md">
                 Apna **Order Number** (6 digits) enter karein jo aapko checkout ke baad mila tha.
              </p>
                   ...
                   >
                      ...
                      <div className="pt-6 border-t border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
                         <div className="flex items-center gap-3">
                            ...
                            <p className="text-xs text-slate-500 font-medium">Delivery Route: <span className="text-slate-800 font-bold">{order.route || 'Local'}</span></p>
                         </div>
