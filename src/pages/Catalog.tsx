...
import { Search, Filter, MessageSquare, Send, X, Package, ArrowRight, ShoppingCart } from 'lucide-react';
...
export default function Catalog() {
  ...
  const handleAiChat = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const context = JSON.stringify(filteredProducts.map(p => ({
        name: p.name, company: p.company, price: p.offerPrice, stock: p.stock
      })));
      ...
      console.error(error);
      setChatHistory(prev => [...prev, { role: 'ai', text: "I'm having trouble connecting to my brain. Please check the API key or try again later." }]);
    } finally {
      setIsTyping(false);
    }
  };
    ...
    <div className="flex flex-col bg-paper">
        ...
        <div className="bg-saffron rounded-[40px] p-12 text-white relative overflow-hidden shadow-2xl shadow-saffron/10">
           <div className="relative z-10">
              ...
              <p className="text-white/80 text-sm max-w-xl">
                 Company select karein · MRP vs best price compare karein · seedha cart mein add karein.
              </p>
           </div>
              ...
              {filteredProducts.map(product => (
                ...
                >
                  ...
                  {product.stock > 0 && product.stock < 10 && (
                    <div className="absolute top-4 right-4 bg-red-500 text-white text-[8px] font-bold px-2 py-1 rounded-full uppercase tracking-widest z-10 animate-pulse">
                       Low Stock
                    </div>
                  )}
                  {product.stock <= 0 && (
      ...
      {!isAiOpen && (
        <button 
          ...
          className="fixed bottom-14 right-10 w-16 h-16 bg-slate-900 border-4 border-white text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all z-40"
        >
          <MessageSquare size={28} />
        </button>
      )}

      {/* AI Assistant Sidebar (kept similar but with Sleek colors) */}
      <AnimatePresence>
        {isAiOpen && (
          <motion.div 
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            exit={{ x: '100%' }}
            className="fixed right-0 top-20 bottom-0 w-[420px] bg-white border-l border-orange-100 shadow-2xl flex flex-col z-50 rounded-tl-[40px] overflow-hidden"
          >
            <div className="p-8 bg-slate-900 text-white flex items-center justify-between">
              ...
              <button onClick={() => setIsAiOpen(false)} className="p-2 hover:bg-white/10 rounded-xl transition-colors"><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-slate-50/30">
              {chatHistory.map((chat, i) => (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={i} className={`flex ${chat.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[90%] px-5 py-4 rounded-[24px] text-sm leading-relaxed shadow-sm ${
              ...
              {isTyping && <div className="flex justify-start"><div className="bg-white px-5 py-4 rounded-full shadow-sm border border-slate-100 flex gap-1">
                {[0, 0.2, 0.4].map(d => <span key={d} className="w-1 h-1 bg-slate-300 rounded-full animate-bounce" style={{animationDelay: `${d}s`}}></span>)}
              </div></div>}
            </div>
            <div className="p-6 bg-white border-t border-slate-100">
              <form onSubmit={handleAiChat} className="flex gap-4">
                ...
                />
                <button className="bg-slate-900 text-white p-4 rounded-2xl shadow-lg hover:bg-saffron transition-all"><Send size={20} /></button>
              </form>
            </div>
          </motion.div>
        )}
