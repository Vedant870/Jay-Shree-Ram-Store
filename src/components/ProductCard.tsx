...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const discount = product.mrp > 0 ? Math.round(((product.mrp - product.offerPrice) / product.mrp) * 100) : 0;
  const isSelectable = product.stock >= minQty;
  ...
  return (
    <motion.div 
      ...
      className={`bg-white p-6 rounded-[32px] border transition-all group relative flex flex-col ${
        !isSelectable ? 'opacity-60 grayscale' : 'shadow-sm border-slate-100 hover:shadow-xl hover:border-saffron/20'
      }`}
    >
      <div className="aspect-square bg-slate-50 rounded-[24px] mb-6 flex items-center justify-center relative overflow-hidden">
        ...
        {!isSelectable && (
          <div className="absolute inset-0 bg-slate-100/40 flex items-center justify-center z-10">
             <span className="bg-red-500 text-white text-[10px] font-bold uppercase tracking-[0.2em] px-4 py-2 rounded-full shadow-xl rotate-[-5deg]">
               {product.stock <= 0 ? 'Khatam (Out of Stock)' : `Min ${minQty} required`}
             </span>
          </div>
        )}
        {product.imageUrl ? (
      ...
      <div className="space-y-1 mb-6">
        ...
        {product.stock > 0 && product.stock < minQty && (
          <p className="text-[9px] text-red-500 font-bold uppercase tracking-widest italic">
            Available stock is below minimum order quantity
          </p>
        )}
      </div>
      ...
      <div className="flex items-end justify-between mt-auto">
        ...
        <div className="flex items-center gap-2">
          {quantity === 0 ? (
            <button 
              onClick={() => isSelectable && increment()}
              disabled={!isSelectable}
              className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg transition-all active:scale-95 ${
                isSelectable ? 'bg-slate-900 text-white hover:bg-saffron' : 'bg-slate-100 text-slate-300 cursor-not-allowed shadow-none'
              }`}
            >
              <Plus size={24} />
            </button>
