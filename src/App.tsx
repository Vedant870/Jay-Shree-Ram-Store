...
import AdminDashboardNew from './pages/admin/AdminDashboard.tsx';
import OrdersPage from './pages/admin/orders/OrdersPage.tsx';
...
const Header = () => {
  ...
  React.useEffect(() => {
    const handleWindowClick = () => setIsLanguageMenuOpen(false);
        ...
        <nav className="flex items-center gap-6 text-[10px] font-bold uppercase tracking-widest text-slate-400">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-4 py-1.5 border border-slate-100 rounded-full hover:border-saffron transition-all"
            >
               <Globe size={12} className="text-saffron" /> {language}
            </button>
            <div className={`absolute top-full right-0 mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl p-2 z-50 min-w-[120px] ${isLanguageMenuOpen ? 'block' : 'hidden'}`}>
          ...
          <Link to="/catalog" className="hover:text-saffron transition-colors">{t('navCatalog')}</Link>
  ...
  >
    {t('navAdminPanel')}
          ...
          <Link to="/track" className="hover:text-saffron transition-colors">{t('navTrackOrder')}</Link>
        </nav>
        ...
        <div className="flex items-center gap-4 pl-10 border-l border-slate-100">
          <Link to="/cart" className="bg-saffron text-white px-6 py-2.5 rounded-full flex items-center gap-3 shadow-lg shadow-saffron/20 hover:scale-[1.02] transition-all">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest">{t('navCart')}</span>
            {count > 0 && (
          ...
          <Link to="/auth" className="p-2.5 border border-slate-100 rounded-full text-slate-400 hover:text-saffron hover:border-saffron transition-all flex items-center gap-2">
            <User size={20} />
            <span className="text-[8px] font-bold uppercase tracking-widest hidden lg:block">{t('navLogin')}</span>
          </Link>
        </div>
      </div>
      ...
      <AnimatePresence>
        {isMobileOpen && (
          ...
          >
            ...
            <Link to="/catalog" onClick={() => setIsMobileOpen(false)} className="hover:text-saffron">{t('navCatalog')}</Link>
            <Link to="/track" onClick={() => setIsMobileOpen(false)} className="hover:text-saffron">{t('navTrackOrder')}</Link>
            <Link to="/admin" onClick={() => setIsMobileOpen(false)} className="text-saffron">{t('navAdminPanel')}</Link>
            <Link to="/cart" onClick={() => setIsMobileOpen(false)} className="text-slate-800">{t('navCart')} ({count})</Link>
          </motion.div>
        )}
      </AnimatePresence>
    </header>
